...
Для выполнения запросов Yupana поддерживает собственный диалект SQL.  Поддерживаются следующие операции:
 ...
 - `SHOW UPDATES_INTERVALS` -- вывод списка временных интервалов для которых были зафиксированы изменения данных временного ряда xxxx (например чеки из прошлого)
