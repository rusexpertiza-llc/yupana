...
class ExpressionCalculatorTest extends WordSpecLike with Matchers {
  ...
  "Expression calculator" should {
    ...
    "Never try to evaluate time, dim, metric or link expressions" in {
      val queryContext = QueryContext(null, None, mutable.HashMap.empty, Array.empty, Array.empty, Array.empty,
        Array.empty, Seq.empty, Array.empty)
      ExpressionCalculator.evaluateExpression(TimeExpr, queryContext, new InternalRow(Array.empty),
        tryEval = true) shouldBe None
      ExpressionCalculator.evaluateExpression(DimensionExpr(Dimension("anyDim")), queryContext,
        new InternalRow(Array.empty), tryEval = true) shouldBe None
      ExpressionCalculator.evaluateExpression(MetricExpr(Metric[Int]("anyMetric", 1)), queryContext,
        new InternalRow(Array.empty), tryEval = true) shouldBe None
      ...
      ExpressionCalculator.evaluateExpression(LinkExpr(TestLink, "foo"), queryContext,
        new InternalRow(Array.empty), tryEval = true) shouldBe None
    }
  }
