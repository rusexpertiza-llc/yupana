...
object PostgresStringReaderWriter extends StringReaderWriter {
  override def readBoolean(s: String): Boolean = s == "t"
  ...
  override def readTime(s: String): Time = ???

  override def writeTime(v: Time): String = ???

  override def readPeriodDuration(s: String): PeriodDuration = ???

  override def writePeriodDuration(v: PeriodDuration): String = ???
