...
class NewParserTest extends FlatSpec with Matchers with Inside with OptionValues {

  import org.yupana.api.query.syntax.All._

  private val parser = new NewParser(TestSchema.schema)

  import TestDims._

  "SqlParser" should "create queries" in {
  ...
  it should "parse function calls" in {
   ...
   WHERE time >= TIMESTAMP '2017-06-12' AND time < TIMESTAMP '2017-06-30' and a = 'AbraCadabra'
   GROUP BY day(time)""",
        parser.statement(parser.nullField)(_),
        true
      )

    val res1 = parse("""GROUP BY day(time)""", parser.groupings(parser.nullField)(_))
...
//      case s: parser.Select => parser.createQuery(s, params)
