...
class TSDaoHBaseTest
  ...
  private def queryContext(table: Table) = {
    val query = Query(Some(table), Seq.empty, None)
    new QueryContext(query, None, JIT, NoMetricCollector)
  }
  ...
  "TSDaoHBase" should "execute time bounded queries" in withMock { (dao, _, queryRunner) =>
    ...
    val rowBuilder = new InternalRowBuilder(exprs.zipWithIndex.toMap, Some(TestSchema.testTable))
    val pointTime = 2000
    ...
    val r = res.head
    r.get[Time](rowBuilder, 0) shouldEqual Time(pointTime)
    r.get[String](rowBuilder, 1) shouldEqual "test1"
    r.get[Double](rowBuilder, 2) shouldEqual 1d
  }
  ...
  it should "support different time conditions" in withMock { (dao, _, queryRunner) =>
    ...
    val r = res.head
    r.get[Time](rowBuilder, 0) shouldEqual Time(pointTime)
    r.get[String](rowBuilder, 1) shouldEqual "test2"
  }
  ...
  it should "use most strict time conditions" in withMock { (dao, _, queryRunner) =>
    ...
    val r = res.head
    r.get[Time](rowBuilder, 0) shouldEqual Time(pointTime)
    r.get[Double](rowBuilder, 1) shouldEqual 3d
  }
  ...
  it should "skip values with fields not defined in schema" in withMock { (dao, _, queryRunner) =>
    ...
    val r1 = res(0)
    r1.get[Time](rowBuilder, 0) shouldEqual Time(pointTime)
    r1.get[String](rowBuilder, 1) shouldEqual "test1"
    r1.get[Short](rowBuilder, 2) shouldEqual 2.toShort
    r1.get[Double](rowBuilder, 3) shouldEqual 3d
    val r2 = res(1)
    r2.get[Time](rowBuilder, 0) shouldEqual Time(pointTime + 1)
    r2.isNull(rowBuilder, 1) shouldBe true // should be "test1" but storage format does not allow this
    r2.get[Short](rowBuilder, 2) shouldEqual 2.toShort // should be "test22" but storage format does not allow this
    r2.isNull(rowBuilder, 3) shouldBe true
  }
  ...
  it should "set tag filter for equ" in withMock { (dao, _, queryRunner) =>
    ...
    r.get[Time](rowBuilder, 0) shouldEqual Time(pointTime)
    r.get[String](rowBuilder, 1) shouldEqual "test1"
    r.get[Short](rowBuilder, 2) shouldEqual 2.toShort
    r.get[Double](rowBuilder, 3) shouldEqual 1d
  }
  ...
  it should "handle tag repr overflow while filtering" in withMock { (dao, _, queryRunner) =>
    ...
    val exprs =
      Seq[Expression[_]](time, dimension(TestDims.DIM_A), dimension(TestDims.DIM_B), metric(TestTableFields.TEST_FIELD))
    val rowBuilder = new InternalRowBuilder(exprs.zipWithIndex.toMap, Some(TestSchema.testTable3))
    val pointTime = 2000
...
//  }
  ...
  it should "support IN operation for tags" in withMock { (dao, _, queryRunner) =>
    ...
    res(0).get[Time](rowBuilder, 0) shouldEqual Time(pointTime1)
    res(0).get[Short](rowBuilder, 1) shouldEqual 5.toShort
    res(0).get[Double](rowBuilder, 2) shouldEqual 7d

    res(1).get[Time](rowBuilder, 0) shouldEqual Time(pointTime2)
    res(1).get[Short](rowBuilder, 1) shouldEqual 5.toShort
    res(1).get[Double](rowBuilder, 2) shouldEqual 5d
  }
  ...
  it should "use post filter if there are too many combinations" in withMock { (dao, _, queryRunner) =>
    ...
    val exprs =
      Seq[Expression[_]](time, dimension(TestDims.DIM_A), dimension(TestDims.DIM_B), metric(TestTableFields.TEST_FIELD))
    val rowBuilder = new InternalRowBuilder(exprs.zipWithIndex.toMap, Some(testTable))
    ...
    res(0).get[Time](rowBuilder, 0) shouldEqual Time(pointTime1)
    res(0).get[String](rowBuilder, 1) shouldEqual "A 1"
    res(0).get[Short](rowBuilder, 2) shouldEqual 2
    res(0).get[Double](rowBuilder, 3) shouldEqual 1d

    res(1).get[Time](rowBuilder, 0) shouldEqual Time(pointTime2)
    res(1).get[String](rowBuilder, 1) shouldEqual "A 1"
    res(1).get[Short](rowBuilder, 2) shouldEqual 2
    res(1).get[Double](rowBuilder, 3) shouldEqual 5d
  }
  ...
  it should "support dimension id not eq" in withMock { (dao, dictionary, queryRunner) =>
    ...
    res(0).get[Short](rowBuilder, 1) shouldEqual 5.toShort
    res(0).get[Double](rowBuilder, 2) shouldEqual 7d
    res(0).get[String](rowBuilder, 3) shouldEqual "Bar"
  }
  ...
  it should "support exact time values" in withMock { (dao, _, queryRunner) =>
    ...
    res.head.get[Time](rowBuilder, 0) shouldEqual Time(pointTime)
    res.head.get[String](rowBuilder, 1) shouldEqual "tag_a"
    res.head.get[Double](rowBuilder, 2) shouldEqual 7d
  }
  ...
  it should "support EQ filter for tuples" in withMock { (dao, _, queryRunner) =>
    ...
    res should have size 1
    res.head.get[Time](rowBuilder, 0) shouldEqual Time(pointTime2)
    res.head.get[String](rowBuilder, 1) shouldEqual "test42"
    res.head.get[Double](rowBuilder, 2) shouldEqual 5d
  ...
  it should "perform pre-filtering by IN for tuples" in withMock { (dao, _, queryRunner) =>
    ...
    res should have size 4
    res.head.get[Time](rowBuilder, 0) shouldEqual Time(pointTime1)
    res.head.get[String](rowBuilder, 1) shouldEqual "test42"
    res.head.get[Double](rowBuilder, 2) shouldEqual 7d

    res(1).get[Time](rowBuilder, 0) shouldEqual Time(pointTime2)
    res(1).get[String](rowBuilder, 1) shouldEqual "test42"
    res(1).get[Double](rowBuilder, 2) shouldEqual 5d

    res(2).get[Time](rowBuilder, 0) shouldEqual Time(pointTime1)
    res(2).get[String](rowBuilder, 1) shouldEqual "test51"
    res(2).get[Double](rowBuilder, 2) shouldEqual 15d

    res(3).get[Time](rowBuilder, 0) shouldEqual Time(pointTime2)
    res(3).get[String](rowBuilder, 1) shouldEqual "test51"
    res(3).get[Double](rowBuilder, 2) shouldEqual 33d
  }
  ...
  it should "handle multiple time bounds" in withMock { (dao, _, queryRunner) =>
    ...
    res.toList.map(r => r.get[Time](rowBuilder, 0)) should have size 2
  }
