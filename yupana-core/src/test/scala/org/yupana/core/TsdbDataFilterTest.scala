...
class TsdbDataFilterTest extends FlatSpec
  with Matchers with TsdbMocks with OptionValues with TableDrivenPropertyChecks with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  it should "execute query with filter by values comparing two ValueExprs" in withTsdbMock { (tsdb, tsdbDaoMock) =>
    ...
    (tsdbDaoMock.query _)
      .expects(
        InternalQuery(
          TestSchema.testTable,
          Set[Expression](time, metric(TestTableFields.TEST_FIELD2), metric(TestTableFields.TEST_FIELD), dimension(TestDims.TAG_A), dimension(TestDims.TAG_B)),
          and(
            ge(time, const(Time(from))),
            lt(time, const(Time(to))),
  ...
  it should "support AND for values, catalogs and tags" in withTsdbMock { (tsdb, tsdbDaoMock) =>
    ...
    (tsdbDaoMock.query _)
      .expects(
        InternalQuery(
          TestSchema.testTable,
          Set[Expression](time, metric(TestTableFields.TEST_STRING_FIELD),metric(TestTableFields.TEST_FIELD), dimension(TestDims.TAG_A), dimension(TestDims.TAG_B)),
          and(
            ge(time, const(Time(from))),
            lt(time, const(Time(to))),
            in(metric(TestTableFields.TEST_FIELD), Set(1012d, 1014d)),
  ...
  it should "support IS NULL for catalog fields" in withTsdbMock { (tsdb, tsdbDaoMock) =>
    ...
    (testCatalogServiceMock.setLinkedValues _)
      ...
      .onCall((qc, datas, _) =>
        setCatalogValueByTag(qc, datas, TestLinks.TEST_LINK, SparseTable("test2a" -> Map("testField" -> "some-value")))
      )
  ...
  it should "support IS NULL and IS NOT NULL for catalog fields within AND among other conditions" in withTsdbMock { (tsdb, tsdbDaoMock) =>
    val testCatalogServiceMock = mockCatalogService(tsdb, TestLinks.TEST_LINK)
    ...
    (testCatalogServiceMock2.setLinkedValues _)
      ...
      .onCall((qc, datas, _) =>
        setCatalogValueByTag(qc, datas, TestLinks.TEST_LINK2,
