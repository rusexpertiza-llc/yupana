...
class NullExpressionCodeGen(override val expression: Expression[_]) extends ExpressionCodeGen[Expression[_]] {
  ...
  override def generateEvalCode(state: State, row: TermName): CodeGenResult = {
    ...
    val validityTree = q"val ${valueDeclaration.validityFlagName} = false"
    val valueTree = q"val ${valueDeclaration.valueName} = ${CommonGen.initVal(expression)}"
