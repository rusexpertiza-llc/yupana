...
4. Apache Spark 2.4.x для запуска запросов на кластере.  Кроме того, в прилагаемых примерах загрузка данных также производится
   из Spark-проложения, хотя это и не является обязательным условием.
...
Для соединения с сервером используя Yupana JDBC используется следующий URL: `jdbc:yupana://localhost:10101`. В качестве
класса драйвера надо указать: `org.yupana.jdbc.YupanaDriver`.
...
Модуль `yupana-examples` может быть использован в качестве основы для создания собственной аналитической системы.  Для этого надо:
...
3. Приведенная в примерах реализация сервера запросов является минимально полной, достаточно использовать схему реализованную
   на шаге 2 схему в сервере.  Однако для интеграции сервера в существую инфраструктуру скорее всего понадобятся некоторые
