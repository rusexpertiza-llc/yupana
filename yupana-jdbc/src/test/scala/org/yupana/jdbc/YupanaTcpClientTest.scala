...
class YupanaTcpClientTest extends FlatSpec with Matchers with OptionValues {
  ...
  "TCP client" should "handle ping/pong" in {
    ...
    val pong = Response(Response.Resp.Pong(Pong(12345678, 12345679, Some(Version(ProtocolVersion.value, 5, 4, "5.4.22")))))
    ...
    req.getPing shouldEqual Ping(12345678, Some(Version(ProtocolVersion.value, BuildInfo.majorVersion, BuildInfo.minorVersion, BuildInfo.version)))
  ...
  it should "fail if protocol version does not match" in {
    val server = new ServerMock
    val client = new YupanaTcpClient("127.0.0.1", server.port)
    val pong = Response(Response.Resp.Pong(Pong(12345678, 12345679, Some(Version(ProtocolVersion.value + 1, 5, 4, "5.4.22")))))
