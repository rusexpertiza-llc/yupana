...
class RuntimeCalculator(tokenizer: Tokenizer) extends ExpressionCalculator with ConstantCalculator with Serializable {

  override def evaluateConstant[T](expr: Expression[T]): T = {
    assert(expr.kind == Const)
    eval(expr, null, null)
  }

  override def evaluateExpression[T](
      expr: Expression[T],
