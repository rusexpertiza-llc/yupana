...
Для выполнения запросов Yupana поддерживает собственный диалект SQL.  Поддерживаются следующие операции:
 ...
 - `SHOW RECALCULATED_PERIODS WHERE ROLLUP_TIME BETWEEN <FROM> AND TIMESTAMP <TO>` -- вывод списка периодов, для которых были пересчитаны роллапы.
 - `SHOW INVALIDATED_BASE_TIMES` -- вывод списка базовых времен, в которые были зафиксированы инвалидирующие чеки (из прошлого).
