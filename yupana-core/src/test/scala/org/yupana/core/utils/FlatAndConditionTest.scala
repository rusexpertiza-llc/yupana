...
class FlatAndConditionTest extends AnyFlatSpec with Matchers with OptionValues {
  ...
  "TimeBoundedCondition" should "handle simple AND conditions" in {
    ...
    val tbcs = FlatAndCondition(calculator, condition, Time(LocalDateTime.now()), Array.empty)
...
//  }
  ...
  it should "merge conditions with same time" in {
    ...
    val from2 = 3000L
    val to2 = 4000L
    val start = Time(LocalDateTime.now())

    FlatAndCondition.mergeByTime(Seq()) shouldBe empty

    FlatAndCondition.mergeByTime(
      Seq(
        FlatAndCondition(from1, to1, Seq(equ(dimension(TestDims.DIM_A), const("x"))), start, Array.empty),
        FlatAndCondition(from1, to1, Seq(equ(dimension(TestDims.DIM_B), const(1.toShort))), start, Array.empty),
        FlatAndCondition(
          from2,
          to2,
        ...
        FlatAndCondition(from1, to2, Seq(in(dimension(TestDims.DIM_A), Set("y"))), start, Array.empty)
      )
