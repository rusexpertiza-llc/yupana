...
class TsdbQueryMetricsDaoHBase(connection: Connection, namespace: String) extends TsdbQueryMetricsDao with StrictLogging {
      ...
      if (n != UPDATE_ATTEMPTS_LIMIT) {
        ...
        queriesByFilter(QueryMetricsFilter(Some(queryId))).headOption match {
          case Some(currentData) =>
            ...
            metricValues.foreach { case (metricName, metricData) =>
              currentData.metrics.get(metricName) match {
                case Some(oldMetricData) =>
                  put.addColumn(FAMILY, Bytes.toBytes(metricName + "_" + metricCount), Bytes.toBytes(oldMetricData.count + metricData.count))
                  put.addColumn(FAMILY, Bytes.toBytes(metricName + "_" + metricTime), Bytes.toBytes(oldMetricData.time + metricData.time))
                case None =>
                  put.addColumn(FAMILY, Bytes.toBytes(metricName + "_" + metricCount), Bytes.toBytes(metricData.count))
            ...
            val result = table.checkAndPut(Bytes.toBytes(queryId), FAMILY, TOTAL_DURATION_QUALIFIER, Bytes.toBytes(currentData.totalDuration), put)
            if (!result) {
