...
class TsdbTcp(tsdb: TSDB,
              ...
              minorVersion: Int,
              version: String
             )(implicit val system: ActorSystem) extends StrictLogging {
  ...
  private implicit val mat: ActorMaterializer = ActorMaterializer(ActorMaterializerSettings(system).withSupervisionStrategy(decider))
  ...
  connections runForeach { conn =>
    ...
    val heartbeat =
      Source.tick(HEART_BEAT_INTERVAL.seconds, HEART_BEAT_INTERVAL.seconds, HEART_BEAT_INTERVAL)
        .scan(0)(_ + _).drop(1)
        .map { time =>
    ...
    val requestFlow = Flow[ByteString].map { b =>
    ...
    .mapAsync(1){
      case Request(Request.Req.Ping(ping)) =>
        if (ping.getVersion.protocol != ProtocolVersion.value) {
          logger.error(s"Incompatible protocols: driver protocol ${ping.getVersion.protocol}, server protocol ${ProtocolVersion.value}")
          Future.failed(new RuntimeException(s"Incompatible protocols: driver protocol ${ping.getVersion.protocol}, server protocol ${ProtocolVersion.value}"))
        } else {
    ...
    val connHandler = protocol
      ...
      .watchTermination() { (_, done) =>
          ...
          case Success(_) =>
            logger.info(s"Response sent to client: ${conn.remoteAddress}, bytes ${humanReadableByteSize(sentSize.get)}, ${sentChunks.get()} chunks")
          case Failure(ex) =>
