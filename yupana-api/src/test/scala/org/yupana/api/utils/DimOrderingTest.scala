...
class DimOrderingTest extends FlatSpec with Matchers with ScalaCheckDrivenPropertyChecks {
  ...
  "DimOrdering" should "support gt" in {
    forAll { (x: Long, y: Long) => ord.gt(x, y) == (java.lang.Long.compareUnsigned(x, y) == 1) }
  }

  it should "support lt" in {
    forAll { (x: Long, y: Long) => ord.lt(x, y) == (java.lang.Long.compareUnsigned(x, y) == -1) }
  }

  it should "support gte" in {
    forAll { (x: Long, y: Long) => ord.gte(x, y) == (java.lang.Long.compareUnsigned(x, y) >= 0) }
  }

  it should "support lte" in {
    forAll { (x: Long, y: Long) => ord.lte(x, y) == (java.lang.Long.compareUnsigned(x, y) <= 0) }
  }
